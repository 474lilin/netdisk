...
### 上传显示「预计完成时间」（剩余时长 + 预计完成时刻）

#### 用户诉求
> 「上传文件时，应该显示预计上传完成时间值。」

#### 改动（`web/src/components/UploadQueue.tsx`）
- 速率改为**指数平滑**（`ema = ema*0.6 + inst*0.4`）：原来用每秒裸差分，抖动会让剩余时间乱跳。
- 新增单任务 ETA：`剩余字节 ÷ 平滑速率`，仅在真正 `uploading` 且速率 > 8KB/s 时给出
  （避免低速时显示"剩余 999 小时"这种噪声值）；任务行显示 `2.35 MB/s · 剩余 50 秒`。
- 新增**队列整体 ETA**：所有活动任务（上传中/哈希中/排队中）剩余字节 ÷ 当前总速率，
  在总进度条与面板页脚显示：`上传 12% · 2.35 MB/s · 剩余约 50 秒（09:06 完成）`——
  同时给出**剩余时长**与**预计完成时刻**（本地时间 HH:MM）。
- 移动端胶囊同样显示 `约 X 分 Y 秒`。
- 新增 `formatDuration()`（秒 → "N 秒 / N 分 M 秒 / N 小时 M 分"）与 `etaClock()`（HH:MM）。
- 任务完成/无速率时自动隐藏 ETA，不残留过期数值。

#### 验证（`e2e/_test-eta.mjs`，真实浏览器 + 真实上传，6/6 通过）
上传 120MB（人为限速）实测：
- 面板出现 `剩余约 50 秒（09:06 完成）` ✅
- 任务行出现 `2.35 MB/s · 剩余 50 秒` ✅
- ETA 随进度动态更新（50 秒 → 6 秒，完成时刻同步前移）✅
- 完成后不再显示剩余时间 ✅ · 服务端大小一致 ✅ · 无 JS 异常 ✅

## [v1.1.17] - 2026-09-18

### 文件句柄续传：刷新后凭浏览器文件句柄继续上传（不限文件大小、无需重新选择文件）

#### 用户诉求
> 「刷新浏览器又导致上传失败……我不要重新选择文件这样的操作。我要点击重试失败就接续上传，
>   边界要求应该不限制文件大小。上传到哪里应该有记录，点击重试失败自动从上次截断的位置接着上传。」

#### 背景与限制（如实说明）
浏览器刷新后**无法保留 `File` 对象**。v1.1.16 用「把文件内容存进 IndexedDB」实现刷新后零点击续传，
但受浏览器配额限制（单文件 1GB / 总量 4GB），大文件仍要重选。**唯一能同时满足"不限大小 + 不重选"的机制**
是 File System Access API 的**文件句柄**：句柄可结构化克隆、可存进 IndexedDB，刷新后凭它重新读取磁盘原文件
（不复制内容、零额外空间），最多弹一次浏览器"允许读取"授权。

#### 改动
1. `resume-store`：新增句柄持久化（`saveFileHandle` / `loadFileHandle` / `fileFromHandle`），
   支持 `queryPermission`/`requestPermission`，并校验大小一致（文件被改过则不当作断点来源）。
2. `store/upload`：`addFiles(files, dirId, handles?)` 保存句柄；新增 `restartTask()`——
   **① 句柄 → ② 浏览器持久化内容 → ③ 才要求选文件**；刷新后先试句柄（非交互，浏览器记住授权即零点击续传）。
3. `FileBrowserPage`：拖拽时用 `DataTransferItem.getAsFileSystemHandle()` 捕获句柄；
   「上传文件」按钮改用系统选择器（可拿句柄），并保留隐藏的普通 `<input type=file>` 作为兼容/回退通道。

#### 已知边界
- 句柄只在**拖拽**或**系统选择器**发起的上传里能拿到；从兼容用普通输入发起的上传拿不到句柄，>1GB 时刷新后仍需选一次。
- 浏览器可能在刷新后要求重新授权（同类"允许读取"提示，非重新选择文件）；选"每次访问都允许"即可零点击。

#### 验证状态（诚实标注）
- ✅ 已自动化验证：≤1GB 文件刷新后**零点击自动续传完成**（40MB 用例）；
  内容被预算淘汰后点「重试失败」会走恢复路径。
- ⚠️ **未验证**：文件句柄路径（浏览器不允许脚本驱动原生文件选择器）与 **15GB 批量刷新**场景。
  15GB 用例在无头浏览器环境下因内存压力中途崩溃（进程被杀），**不构成通过结论**，
  需由使用者在真实浏览器中拖拽验证。

## [v1.1.16] - 2026-09-18

### 刷新浏览器不再打断上传；重试方式从「重新选择文件」改为「重新开始」

#### 用户诉求
> 「有上传任务时刷新浏览器 web 界面就导致上传失败；重新上传要逐个对照重新选择文件，非常不方便。
>   改成点击『重新开始』就能接着上传，同时把刷新这个 bug 也修掉。」

#### 根因
浏览器不允许把 `File` 对象写进 localStorage，旧实现只在 **≤8MB** 小文件时才把文件内容存进 IndexedDB
（`FILE_PERSIST_LIMIT`），大文件刷新后只剩"名字 + 大小" → 必须由用户重新选择文件，且**逐个**选。

#### 改动
1. **文件内容持久化（核心）**：`resume-store` 不再限制 8MB——**只要单文件 ≤1GB 就把内容一起存进
   IndexedDB**（结构化克隆原生支持 File/Blob），并对所有记录的**内容总量**设 4GB 预算，
   超预算时按最旧优先丢弃内容副本（断点进度保留）。上传引擎的分片/单请求两条路径都改为总是携带 File。
2. **刷新后自动续传（bug 修复）**：`hydrateTaskList` 与 `useAutoResume` 不再按大小过滤，
   只要记录里还有文件内容就**自动入队续传**——用户刷新后什么都不用点。
3. **「重新开始」取代「重新选择文件」**：新增 store 动作 `restartTask()`：
   ① 先用浏览器里持久化的内容直接续传（零操作）；② 内容确实不在时才返回 `need-file` 让 UI 弹选择框。
   任务行按钮改为「重新开始」；批量按钮「重新开始全部」会先自动续传能续的，剩下的**一次性多选**补上。
4. **防误刷新**：有任务在上传/排队时注册 `beforeunload`，刷新会先弹浏览器确认，避免手滑丢失上传。
5. 文案更新：中断提示改为「点『重新开始』即可接着传（浏览器若仍留有该文件内容会直接续传……）」。

#### 已知边界（如实说明）
- **超过 1GB 的单个文件不存内容**，刷新后仍需选回该文件（浏览器配额有限，避免把 C 盘吃满）；
  若总量超 4GB，最旧的副本会被淘汰，同样需要选回。
- 持久化内容占用**浏览器所在盘（C:）**的空间，请在 C: 保留足够余量。

#### 验证（`e2e/_test-resume-after-refresh.mjs`，真实浏览器 + 真实刷新，9/9 通过）
- **场景 A**：40MB 上传到 70% 时刷新页面 → **不做任何点击**，任务自动续传至 completed ✅
  （断点记录显示已持久化内容 + 已传 3 个分片）
- **场景 B**：抹掉浏览器里的内容副本（模拟预算淘汰）→ 任务显示「需要重新开始」→
  点「重新开始」才弹文件选择框 → 选回后完成上传 ✅；无 JS 异常 ✅
- 回归：UI 全功能体检 **33/33** · 秒传守卫 ✅ · 清除失败按钮 ✅
...
### 秒传守卫：对象不存在时不再误判「秒传成功」

#### 背景
2026-09-17 的存储事故排查中发现：`initUpload` 遇到「同名 + 同内容哈希」会**直接返回 `dedup: true`**，
从不校验 MinIO 里的对象是否真的存在。一旦对象因误删/清理/迁移而缺失，用户会看到"上传完成"，
但下载 404 —— 在"用本地原件重传以恢复网盘"的场景下会 **100% 踩中**（重传被秒传骗过，什么都没恢复）。

#### 改动
- `server/src/services/file.service.ts`：同名同哈希分支在返回秒传前先 `objectExists(existing.object_key)`；
  对象缺失则打 `warn` 日志并**继续走真实上传**（覆盖语义下用的还是同一个 object_key）。
- 池命中路径（`tryDedupHit`）本就已校验 `objectExists(poolKey)`，无需改动。

#### 验证
`e2e/_test-dedup-guard.mjs`（新增）：首次上传 → 同名再传（期望秒传 `dedup=true`，正常路径不受影响）→
用 `mc rm` 直接删掉 MinIO 对象 → 同名再传（**期望 `dedup=false` 走真实上传**，守卫生效）→
对象回来 → 下载内容逐字节一致。**全部通过 ✅**
